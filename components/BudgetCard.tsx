...
const BudgetCard: React.FC<BudgetCardProps> = ({ totalBudget, totalSpent }) => {
  ...
  return (
    <div className="bg-brand-card-bg p-6 rounded-lg shadow-sm border-2 border-brand-light-blue md:col-span-2 lg:col-span-1">
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-md font-semibold text-brand-text-dark">Total 3-Year Budget:</h3>
          <p className="text-3xl font-bold text-brand-text-dark my-1">{formatCurrency(totalBudget)}</p>
          <p className="text-sm text-gray-500">Total Spent YTD: <span className="font-semibold">{formatCurrency(totalSpent)}</span></p>
        </div>
